...
def process_frame_with_ocr(frame_path, output_dir):
...
def detect_text_in_frames(temp_dir, output_dir, batch_size=16):
    ...
    # Processar os frames em lotes usando ThreadPoolExecutor
    with ThreadPoolExecutor(max_workers=batch_size) as executor:
        future_to_frame = {executor.submit(process_frame_with_ocr, frame, output_dir): frame for frame in frame_files}

        for future in tqdm(as_completed(future_to_frame), total=len(future_to_frame), desc="Detectando texto nos frames", unit="frame"):
            try:
                future.result()  # Obtém o resultado do processamento do frame
            except Exception as e:
                print(f"Erro ao processar lote de frames: {e}")
...
def process_frames(video_path, temp_dir, progress_queue):
    ...
    try:
        ...
        detect_text_in_frames(temp_dir, output_dir, batch_size=16)
