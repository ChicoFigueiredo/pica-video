...
def is_human_readable_text(text):
    """Verifica se o texto contém palavras legíveis baseadas nos critérios solicitados:
       - Pelo menos uma palavra com 4 letras ou mais OU
       - Mais de uma palavra com pelo menos 3 letras."""
    
    words = text.split()
    long_words = [word for word in words if len(word) >= 4]
    medium_words = [word for word in words if len(word) >= 3]
    
    # Critério 1: Pelo menos uma palavra com 4 letras ou mais
    if len(long_words) > 0:
        return True
    
    # Critério 2: Mais de uma palavra com 3 letras ou mais
    if len(medium_words) > 1:
        return True
...
def main():
    ...
    parser.add_argument("--temp_dir", type=str, default="/mnt/t/temp", help="Pasta temporária para armazenar frames.")
