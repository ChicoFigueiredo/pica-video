...
def processar_frames(caminho_video, pasta_saida, fila_progresso):
    ...
    #            logging.info(f"Pasta de saída {pasta_saida} removida.")
    #    except Exception as e:
    #        logging.warning(f"Não foi possível remover a pasta de saída {pasta_saida}: {e}")
...
def transcrever_audio(caminho_video, nome_modelo="medium", idioma="Portuguese"):
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            logging.info(f"Iniciando Transcrição do vídeo usando o Whisper")

            modelo_whisper = whisper.load_model(nome_modelo)
            resultado = modelo_whisper.transcribe(caminho_video, language=idioma, task="transcribe", verbose=True)
...
def main():
    ...
    parser.add_argument("--modelo", type=str, default="small", help="Modelo Whisper a ser utilizado (padrão: small).")
