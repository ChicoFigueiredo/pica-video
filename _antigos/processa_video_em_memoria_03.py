...
def processar_frames_em_memoria(caminho_video, pasta_saida):
    ...
    try:
        # Obter o tamanho total do arquivo de vídeo
        tamanho_total_video = os.path.getsize(caminho_video)
        bytes_processados = 0
        ...
        processo = subprocess.Popen(comando, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=10**8)

        frame_num = 0
        pbar = tqdm(total=tamanho_total_video, desc="Processando frames", unit="B", unit_scale=True)
        ...
        while True:
            ...
            bytes_processados += len(cabecalho)
            ...
            while True:
                byte = processo.stdout.read(1024)
                if not byte:
                    break
                dados_imagem += byte
                bytes_processados += len(byte)
                # Verificar o final do arquivo PNG (IEND)
                if b'IEND' in dados_imagem[-12:]:
                    break
            ...
            try:
                ...
                pbar.update(bytes_processados - pbar.n)

        pbar.close()

        # Verificar se houve erros no ffmpeg
        stderr = processo.stderr.read()
        if stderr:
            logging.error(f"Erro do ffmpeg: {stderr.decode('utf-8')}")
