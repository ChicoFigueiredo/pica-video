...
def texto_legivel(texto, porcentagem_minima=40.0, min_palavra=3):
    """Verifica se o texto é legível e está em português, baseado na porcentagem mínima de palavras legíveis e tamanho mínimo de palavras."""
    try:
        ...
        palavras_desconhecidas = corretor.unknown(palavras)
        palavras_legiveis = total_palavras - len(palavras_desconhecidas)

        porcentagem_legiveis = (palavras_legiveis / total_palavras) * 100

        # Verificar se a porcentagem de palavras legíveis é suficiente
        if porcentagem_legiveis >= porcentagem_minima:
            # Unir as palavras para detecção de idioma
            texto_para_deteccao = ' '.join(palavras)
            # Detectar idioma
...
def processar_frames_com_opencv(caminho_video, pasta_saida, fps_extracao=4, porcentagem_minima=40.0, min_palavra=3):
    """Processa os frames do vídeo usando OpenCV e salva apenas aqueles com texto legível."""
    try:
        ...
        while True:
            ...
            if frame_num % frame_intervalo == 0:
                try:
                    ...
                    configuracao_tesseract = r'--oem 1 --psm 6 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 -l por'

                    # Executar OCR
                    texto = pytesseract.image_to_string(imagem_proc, config=configuracao_tesseract)

                    if texto.strip() and texto_legivel(texto, porcentagem_minima, min_palavra):
...
def main():
    ...
    parser.add_argument("--porcentagem_legivel", type=float, default=40.0, help="Porcentagem mínima de palavras legíveis (padrão: 40%).")
    parser.add_argument("--min_palavra", type=int, default=3, help="Tamanho mínimo das palavras a serem consideradas (padrão: 3).")
    args = parser.parse_args()

    mascara_arquivos = args.mascara_arquivos
    nome_modelo = args.modelo
    recursivo = args.recursivo
    porcentagem_minima = args.porcentagem_legivel
    ...
    for caminho_video in arquivos_video:
        try:
            ...
            processo_frames = threading.Thread(target=processar_frames_com_opencv, args=(caminho_video, pasta_saida, 4, porcentagem_minima, min_palavra))
