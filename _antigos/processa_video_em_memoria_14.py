...
def detectar_texto(frame, net, min_confidence=0.5):
    ...
    try:
        ...
        if len(rects) > 0:
            ...
            if len(indices) > 0:
                for idx in indices:
                    # Dependendo da versão do OpenCV, idx pode ser uma lista ou um inteiro
                    if isinstance(idx, (list, tuple, np.ndarray)):
                        idx = idx[0]
                    rect = rects[idx]
                    # Obter os vértices da caixa rotacionada
...
def decode_predictions(scores, geometry, min_confidence):
    ...
    for y in range(0, numRows):
        ...
        for x in range(0, numCols):
            ...
            # Calcular centro
            centerX = offsetX + (cos * x1[x]) + (sin * x2[x])
            centerY = offsetY - (sin * x1[x]) + (cos * x2[x])

            rect = ((centerX, centerY), (w, h), -angle * 180.0 / np.pi)
            rects.append(rect)
...
def preprocessar_imagem(imagem):
    """
    Preprocessa a imagem para melhorar a precisão do OCR em fundos coloridos.
    """
    try:
        # Aplicar filtro bilateral para reduzir ruído e manter as bordas
        imagem_cv = cv2.cvtColor(np.array(imagem), cv2.COLOR_RGB2BGR)
        imagem_cv = cv2.bilateralFilter(imagem_cv, 9, 75, 75)
        ...
        # Equalizar o histograma do canal de brilho
        v_eq = cv2.equalizeHist(v)

        # Recompor a imagem HSV e converter de volta para BGR
        hsv_eq = cv2.merge((h, s, v_eq))
        imagem_eq = cv2.cvtColor(hsv_eq, cv2.COLOR_HSV2BGR)

        # Converter para escala de cinza
        imagem_gray = cv2.cvtColor(imagem_eq, cv2.COLOR_BGR2GRAY)

        # Aplicar limiarização adaptativa
        imagem_thresh = cv2.adaptiveThreshold(
        ...
        return imagem  # Retorna a imagem original em caso de erro

def processar_frames_com_opencv(
    caminho_video, pasta_saida, fps_extracao=4, min_palavra=4
):
                ...
                try:
                    ...
                    if caixas:
                        ...
                        for vertices in caixas:
                            ...
                            imagem_proc = preprocessar_imagem(imagem_pil)
                            # Executar OCR na ROI
                            configuracao_tesseract = (
                                r"--oem 3 --psm 11 -l por"
                            )
                            texto = pytesseract.image_to_string(
                                imagem_proc, config=configuracao_tesseract
                            )
                            texto_total += " " + texto
                        ...
                        if texto_total.strip() and texto_legivel(texto_total, min_palavra):
                            ...
                            caminho_texto_saida = os.path.join(
                                pasta_saida, f"frame_{timestamp_formatado}.txt"
                            )
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            resultado = modelo_whisper.transcribe(
                caminho_video, language=idioma, task="transcribe"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Processa vídeos para extrair frames com texto e gerar legendas com Whisper."
    )
    parser.add_argument(
        "mascara_arquivos",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Tamanho mínimo das palavras a serem consideradas (padrão: 4).",
    )
    ...
    for caminho_video in arquivos_video:
        try:
            ...
            processo_frames = threading.Thread(
                ...
                args=(caminho_video, pasta_saida, 4, min_palavra),
            )
