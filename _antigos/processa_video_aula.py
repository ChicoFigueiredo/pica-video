...
def is_human_readable_text(text):
    """Verifica se o texto contém pelo menos algumas sílabas legíveis para humanos."""
    words = text.split()
    syllables = 0
    ...
    return syllables >= 3  # Consideramos 3 sílabas como critério mínimo para texto legível
...
def main():
    ...
    try:
        ...
        for video_path in video_files:
            with tempfile.TemporaryDirectory(dir=os.path.dirname(video_path), prefix=os.path.basename(video_path).replace(".mp4", "") + "_") as temp_dir:
