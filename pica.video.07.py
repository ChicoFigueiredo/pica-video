...
def transcrever_audio_faster_whisper(caminho_video, modelos=["large-v3"], idiomas=['pt']):
    """Transcreve áudio do vídeo usando o Faster-Whisper para múltiplos idiomas."""
    try:
        for nome_modelo, idioma in zip(modelos, idiomas):
            logging.info(f"")
            logging.info(f"Iniciando Transcrição do vídeo {caminho_video} usando o Faster-Whisper no idioma: {idioma}")
            logging.info(f"===============================================================================================================")

            modelo_whisper = WhisperModel(model_size_or_path=nome_modelo, device="cpu", compute_type="int8")
            logging.debug(f"Modelo {nome_modelo} carregado com sucesso.")

            segmentos, info = modelo_whisper.transcribe(caminho_video, beam_size=5, language=idioma)

            logging.debug(f"Transcrição completa para {idioma}. Info: {info}")

            caminho_srt = caminho_video.replace(".mp4", f"_{idioma}.srt")
            caminho_fala_cronometrada = caminho_video.replace(".mp4", f"-Fala.Cronometrada.{idioma}.txt")

            logging.info(f"Salvando SRT / Fala Cronometrada para {idioma}")
            with open(caminho_srt, "w", encoding="utf-8") as arquivo_srt, \
                 open(caminho_fala_cronometrada, "w", encoding="utf-8") as arquivo_txt:
            ...
            logging.info(f"Arquivos de transcrição gerados para {idioma}: {caminho_srt}, {caminho_fala_cronometrada}")
    except Exception as e:
        logging.error(f"Erro ao transcrever áudio para {caminho_video}: {e}", exc_info=True)
        raise
...
def processar_transcricao(caminho_video, fila_progresso, modelos=['large-v3'], idiomas=['pt'], ):
    """Processa a transcrição de áudio."""
    try:
        transcrever_audio_faster_whisper(caminho_video, modelos=modelos, idiomas=idiomas)
...
def main():
    ...
    parser.add_argument("--idioma-original", type=str, default="large-v3", help="Se o arquivo não é no idioma original, coloque aqui.")
    ...
    for caminho_video in encontrar_arquivos_mascara(mascara_arquivos, recursivo):
        try:
            ...
            if skip_prontos and os.path.exists(caminho_fala_cronometrada) and os.path.getsize(caminho_fala_cronometrada) > 1024:
                logging.info(f"Pulado: {caminho_video}, arquivo de fala cronometrada já existe e é maior que 1KB.")
            ...
            if args.idioma_original:
                idiomas = [args.idioma_original, 'pt']
                modelos = [nome_modelo, nome_modelo]
            else:
                idiomas = ['pt']
                modelos = [nome_modelo]

            # Criar e iniciar processo de transcrição
            processo_transcricao = Process(target=processar_transcricao, args=(caminho_video, fila_progresso_transcricao, modelos, idiomas))
            processo_transcricao.start()

            # Iniciar processo de frames apenas se não estiver desativado
            if not desativar_frames:
                ...
                logging.info("Processamento de frames desativado.")

            # Monitorar progresso
            while processo_transcricao.is_alive() or (not desativar_frames and processo_frames.is_alive()):
                if not desativar_frames and not fila_progresso_frames.empty():
